...
describe('Initialization', () => {
  context('credentials', () => {
    it('throws error if missing credentials', async () => {
      expect(
        AfricasTalking(invalidCredentials1 as any).SMS.send({} as any),
      ).to.be.rejected;
    });

    it('throws error if invalid credentials', async () => {
      expect(
        AfricasTalking(invalidCredentials2 as any).SMS.send({} as any),
      ).to.be.rejected;
    });

    it('passes credential validation', () => {
      expect(
        AfricasTalking(validCredentials).SMS.send({} as any),
