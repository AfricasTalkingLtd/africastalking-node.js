...
class Airtime {
    ...
    send(options) {
        return new Promise((resolve, reject) => {
            ...
            const { recipients: rawRecipients } = value.recipients;
            const recipients = rawRecipients.map(r => ({
                phoneNumber: r.phoneNumber,
                amount: `${r.currencyCode} ${r.amount}`,
            }));
            const customAxios = value.idempotencyKey ? initializeAxios(this.config, idempotencyKey) : initializeAxios(this.config);
