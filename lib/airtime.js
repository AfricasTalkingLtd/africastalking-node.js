...
class Airtime {
    ...
    send(options) {
     ...
     findTransactionStatus(options){
        return new Promise((resolve, reject) => {
            const { error, value } = this.validationOptions({ context:'findTransactionStatus', options});
            if (error) {
                const combinedMessages = error.details.map(d => d.message).join(';');
                reject(new Error(combinedMessages));
                return;
    ...
    validateOptions({context, options}) {
        ...
        findTransactionStatus: Joi.object().keys({
            transactionId: Joi.string().required(),
        }).required(),
    }

        if(!schema[context]) throw new Error(`Schema context '${context}' does not exist`);

        return schema[context].validate(options);
    }
}
