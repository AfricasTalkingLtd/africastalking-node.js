...
router.post('/', (req, res) => {
  ...
  // Prep state
  const state = isActive === '1' ? session : ''

  switch (state) {
    case 'menu':
      session = 'process'
      responseActions = `<Say>Hello bot ${callerNumber || 'There'}</Say>
  ...
  const response = `<?xml version="1.0" encoding="UTF-8"?><Response>${responseActions}</Response>`
